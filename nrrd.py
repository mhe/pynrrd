...
def read_header(nrrdfile):
    ...
    headerSize += _validate_magic_line(next(it).decode(sys.getdefaultencoding()))

    header = { u'keyvaluepairs': {} }
    for raw_line in it:
        headerSize += len(raw_line)
        raw_line = raw_line.decode(sys.getdefaultencoding())
...
def write(filename, data, options={}, detached_header=False):
    ...
    with open(filename,'wb') as filehandle:
        ...
        filehandle.write(b'# on ' +
                         datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S').encode(sys.getdefaultencoding()) +
                         b'(GMT).\n')
