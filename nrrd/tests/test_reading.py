...
class TestReadingFunctions:
    def setUp(self):
        ...
        self.expected_data = np.fromfile(RAW_DATA_FILE_PATH, np.int16).reshape(
            (30, 30, 30)
        )
    ...
    def test_read_detached_header_and_data_with_byteskip_minus1(self):
        ...
        data, header = nrrd.read(
            RAW_BYTESKIP_NHDR_FILE_PATH, index_order=self.index_order
        )
    ...
    def test_read_detached_header_and_nifti_data_with_byteskip_minus1(self):
        ...
        data, header = nrrd.read(
            GZ_BYTESKIP_NIFTI_NHDR_FILE_PATH, index_order=self.index_order
        )
    ...
    def test_read_header_and_gz_compressed_data_with_byteskip_minus1(self):
        ...
        data, header = nrrd.read(
            GZ_BYTESKIP_NRRD_FILE_PATH, index_order=self.index_order
        )
    ...
    def test_read_header_and_gz_compressed_data_with_lineskip3(self):
        ...
        data, header = nrrd.read(
            GZ_LINESKIP_NRRD_FILE_PATH, index_order=self.index_order
        )
    ...
    def test_read_raw_header(self):
        ...
        header = nrrd.read_header(
            ("NRRD0005", "type: float", "dimension: 3", "min: 0", "max: 35.4")
        )
        self.assertEqual(expected_header, header)

        expected_header = {"my extra info": "my : colon-separated : values"}
        header = nrrd.read_header(
            ("NRRD0005", "my extra info:=my : colon-separated : values")
        )
    ...
    def test_read_header_and_ascii_2d_data(self):
        ...
        np.testing.assert_equal(
            data, np.arange(1, 28).reshape(expected_shape, order=self.index_order)
        )
    ...
    def test_read_simple_4d_nrrd(self):
        expected_header = {
            ...
            "endian": "little",
            "encoding": "raw",
            "measurement frame": np.array(
                [
                    [1.0001, 0.0, 0.0],
                    [0.0, 1.0000000006, 0.0],
                    [0.0, 0.0, 1.000000000000009],
                ]
            ),
        }
    ...
    def test_custom_fields_with_field_map(self):
        expected_header = {
            ...
            "double matrix": np.array(
                [[1.2, 0.3, 0.0], [0.0, 1.5, 0.0], [0.0, -0.55, 1.6]]
            ),
            "type": "unsigned char",
        }
    ...
    def test_invalid_magic_line(self):
        with self.assertRaisesRegex(
            nrrd.NRRDError, "Invalid NRRD magic line. Is this an NRRD file?"
        ):
            nrrd.read_header(
                ("invalid magic line", "my extra info:=my : colon-separated : values")
            )
    ...
    def test_invalid_magic_line3(self):
        with self.assertRaisesRegex(
            nrrd.NRRDError, "Invalid NRRD magic line: NRRDnono"
        ):
            nrrd.read_header(
                ("NRRDnono", "my extra info:=my : colon-separated : values")
            )
    ...
    def test_missing_required_field(self):
        with open(RAW_NRRD_FILE_PATH, "rb") as fh:
            ...
            with self.assertRaisesRegex(
                nrrd.NRRDError, 'Header is missing required field: "type".'
            ):
    ...
    def test_missing_endianness(self):
        with open(RAW_NRRD_FILE_PATH, "rb") as fh:
            ...
            with self.assertRaisesRegex(
                nrrd.NRRDError, 'Header is missing required field: "endian".'
            ):
                nrrd.read_data(header, fh, RAW_NRRD_FILE_PATH)
