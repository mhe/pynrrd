...
def _handle_header_fields(data, header=None, index_order='F'):
    if header is None:
        header = {}
...
def _write_default_header_entries(file):
...
def _write_header(file, header, custom_field_map=None):
    _write_default_header_entries(file)
...
def write(file, data, header=None, detached_header=False, relative_data_path=True, custom_field_map=None,
    ...
    relative_data_path : :class:`bool`
        Whether the data file in detached header is saved with a relative path or absolute path.
    ...
    header = _handle_header_fields(data, header, index_order)

    # If the file is a file handle rather than filename, then write to the file.
