...
def write(filename, data, header={}, detached_header=False, custom_field_map=None):
    ...
    if detached_header:
        with open(data_filename, 'wb') as datafilehandle:
            _write_data(data, datafilehandle, header)


def _write_data(data, fh, options):
    ...
    if options['encoding'] == 'raw':
        ...
        while start_index < len(rawdata):
            ...
            fh.write(comp_obj.compress(rawdata[start_index:end_index]))
            start_index = end_index
        fh.write(comp_obj.flush())
        fh.flush()
